...
export class LazyCollection<T> implements Iterable<T> {
    ...
    takeUntil(valueOrCallback: T | ((item: T, index: number) => boolean)): LazyCollection<T> {
        ...
        return new LazyCollection(function* () {
            ...
            while (true) {
                const { done, value } = iter.next();
                if (done) break;
                const match = typeof valueOrCallback === "function" ? (valueOrCallback as (item: T, index: number) => boolean)(value, i++) : value === valueOrCallback;
                if (match) break;
                yield value;
            }
        });
    ...
    sum(key?: keyof T | ((item: T) => number)): number {
        let total = 0;
        for (const item of this) {
            const v = key ? (typeof key === "function" ? key(item) : (item[key] as unknown as number)) : (item as unknown as number);
            total += v;
        }
        return total;
    }
    ...
    groupBy<K extends string>(key: keyof T | ((item: T) => K)): Record<string, LazyCollection<T>> {
        ...
        return Object.fromEntries(
            Object.entries(groups).map(([k, v]) => [k, LazyCollection.make(v)]),
        );
    }
