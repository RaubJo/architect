...
export function snake(value: string, separator = "_"): string {
    return splitWords(value).map((w) => w.toLowerCase()).join(separator);
}
...
export function isUuid(value: string, version?: 1 | 3 | 4 | 5 | 6 | 7 | 8): boolean {
    ...
    return new RegExp(
        `^[0-9a-f]{8}-[0-9a-f]{4}-${v}[0-9a-f]{3}-[0-9a-f]{4}-[0-9a-f]{12}$`,
        "i",
    ).test(value);
}
...
export function padBoth(value: string, padLength: number, pad = " "): string {
    ...
    return pad.repeat(Math.ceil(leftPad / pad.length)).slice(0, leftPad) +
        value +
