...
function fnName(node, sf) {
    ...
    const p = node.parent
    const varName = p && ts.isVariableDeclaration(p) && p.name ? p.name.getText(sf) : null
    if (ts.isArrowFunction(node)) return varName ?? "<arrow>"
    if (ts.isFunctionExpression(node)) return varName ?? "<anonymous>"
    return "<anonymous>"
}
