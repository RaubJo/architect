...
describe("Str helper", () => {
    ...
    test("limit truncates to a character count", () => {
        expect(Str.limit("The quick brown fox jumped over the lazy dog", 20)).toBe("The quick brown fox ...")
    ...
    test("remove deletes occurrences of a search string", () => {
        ...
        expect(Str.remove("E", "Peter Piper Picked Peppers", false)).toBe("Ptr Pipr Pickd Ppprs")
    ...
    test("replaceMatches replaces using a pattern and string or callback", () => {
        expect(Str.replaceMatches("/[^A-Za-z0-9]+/", "", "Hello, World! 123")).toBe("HelloWorld123")
        expect(Str.replaceMatches(/(\d+)/, (m) => String(Number(m[1]) * 2), "1 and 2 and 3")).toBe("2 and 4 and 6")
    })
    ...
    test("swap replaces keys with values (longest key first)", () => {
        expect(Str.swap({ Tacos: "Burritos", great: "fantastic" }, "Tacos are great!")).toBe("Burritos are fantastic!")
    })
    ...
    test("wordWrap wraps long lines at word boundaries", () => {
        ...
        expect(Str.wordWrap("The quick brown fox", 15, "<br>")).toBe("The quick brown<br>fox")
    })

    // ── words ─────────────────────────────────────────────────────────────────

    test("words limits to a number of words", () => {
        expect(Str.words("Perfectly balanced, as all things should be.", 3)).toBe("Perfectly balanced, as...")
