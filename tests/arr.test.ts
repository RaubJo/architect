...
describe("Arr", () => {
    ...
    test("divide", () => {
        expect(Arr.divide({ a: 1, b: 2 })).toEqual([["a", "b"], [1, 2]]);
    });
    ...
    test("keyBy", () => {
        const result = Arr.keyBy([{ id: 1, name: "a" }, { id: 2, name: "b" }], "id");
    ...
    test("mapWithKeys", () => {
        const result = Arr.mapWithKeys([{ id: 1, v: "a" }, { id: 2, v: "b" }], (item) => [
            String(item.id),
            item.v,
        ]);
    ...
    test("select", () => {
        const result = Arr.select([{ a: 1, b: 2 }, { a: 3, b: 4 }], ["a"]);
