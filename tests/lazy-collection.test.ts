...
describe("LazyCollection", () => {
    ...
    test("map transforms each item", () => {
        expect(LazyCollection.make([1, 2, 3]).map((n) => n * 2).all()).toEqual([2, 4, 6]);
    });

    test("map passes index", () => {
        expect(LazyCollection.make(["a", "b", "c"]).map((_, i) => i).all()).toEqual([0, 1, 2]);
    });

    // ── filter ────────────────────────────────────────────────────────────────

    test("filter keeps matching items", () => {
        expect(LazyCollection.make([1, 2, 3, 4]).filter((n) => n % 2 === 0).all()).toEqual([2, 4]);
    });
    ...
    test("reject drops matching items", () => {
        expect(LazyCollection.make([1, 2, 3, 4]).reject((n) => n % 2 === 0).all()).toEqual([1, 3]);
    });

    // ── flatMap ───────────────────────────────────────────────────────────────

    test("flatMap maps and flattens one level", () => {
        expect(LazyCollection.make([1, 2, 3]).flatMap((n) => [n, n * 10]).all()).toEqual([1, 10, 2, 20, 3, 30]);
    });
    ...
    test("takeUntil takes until callback match", () => {
        expect(LazyCollection.make([1, 2, 3, 4]).takeUntil((n) => n >= 3).all()).toEqual([1, 2]);
    });

    test("takeWhile takes while condition holds", () => {
        expect(LazyCollection.make([1, 2, 3, 4]).takeWhile((n) => n < 3).all()).toEqual([1, 2]);
    });
    ...
    test("skipUntil skips until callback match", () => {
        expect(LazyCollection.make([1, 2, 3, 4]).skipUntil((n) => n > 2).all()).toEqual([3, 4]);
    });

    test("skipWhile skips while condition holds", () => {
        expect(LazyCollection.make([1, 2, 3, 4]).skipWhile((n) => n < 3).all()).toEqual([3, 4]);
    });
    ...
    test("concat appends another iterable", () => {
        ...
        expect(LazyCollection.make([1]).concat(LazyCollection.make([2, 3])).all()).toEqual([1, 2, 3]);
    });
    ...
    test("groupBy returns grouped LazyCollections", () => {
        ...
        expect(groups["a"].all()).toEqual([{ type: "a", v: 1 }, { type: "a", v: 3 }]);
        expect(groups["b"].all()).toEqual([{ type: "b", v: 2 }]);
    });

    test("groupBy with callback", () => {
        const groups = LazyCollection.make([1, 2, 3, 4]).groupBy((n) => (n % 2 === 0 ? "even" : "odd") as "even" | "odd");
    ...
    test("pluck extracts a key from each item", () => {
        expect(LazyCollection.make([{ name: "a" }, { name: "b" }]).pluck("name").all()).toEqual(["a", "b"]);
    });
