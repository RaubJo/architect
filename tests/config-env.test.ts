...
describe("env helper", () => {
    ...
    const originalImportMetaEnv = (globalThis as { __iocImportMetaEnvForTests?: unknown }).__iocImportMetaEnvForTests

    afterEach(() => {
        ;(globalThis as { process?: unknown }).process = originalProcess
        ;(globalThis as { env?: unknown }).env = originalGlobalEnv
        ;(globalThis as { __iocImportMetaEnvForTests?: unknown }).__iocImportMetaEnvForTests = originalImportMetaEnv
    })
    ...
    test("returns default when key does not exist", () => {
        ...
        ;(globalThis as { __iocImportMetaEnvForTests?: unknown }).__iocImportMetaEnvForTests = undefined
    ...
    test("prefers import-meta env when key exists there", () => {
        ;(globalThis as { __iocImportMetaEnvForTests?: Record<string, unknown> }).__iocImportMetaEnvForTests = {
            CACHE_STORE: "(true)",
        }
        ;(globalThis as { process?: { env: Record<string, unknown> } }).process = {
            env: {
